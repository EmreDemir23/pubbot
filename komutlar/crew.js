...
exports.run = async (client, message, args) => {
  ...
  if(!message.member.roles.get("") &&!message.member.hasPermission('ADMINISTRATOR')) return message.channel.send("Bu Komutu Kullanmaya Yetkin Yok!!").then(m => m.delete(5000));
...
if(!user) return message.reply ("Lütfen bir kullanıcı etiketleyiniz").then(m => m.delete(5000));
  ...
  if(!user.roles.has(Crew.id)){
      ...
      if(user.user.username.includes("! NΣFΣЯ ♔")){
        let değişeceksembol2 = takmaad.replace(/★/g, "! NΣFΣЯ ♔");
...
**`)
  ...
  message.channel.send("Etiketlenen Kullanıcının İsminde **! NΣFΣЯ ♔** Yok!").then(m => m.delete(10000));
}
      ...
      if(!user.user.username.includes("☆")){
         let değişeceksembol1 = takmaad.replace(/☆/g, "! NΣFΣЯ ♔");
...
**`)
  ...
  message.channel.send("Etiketlenen Kullanıcının İsminde Zaten **! NΣFΣЯ ♔** Var!").then(m => m.delete(10000));
}
